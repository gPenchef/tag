...
`shared/game-config.json` is the single source of truth shared by the server and browser. `shared/game-config.js` loads that JSON for CommonJS code, while `server.js` exposes the same data at `/game-config.js` for the browser.

Each map in `maps` contains its own complete geometry:
