...
function exitCurrentLobby(socket, disconnected = false) {
  ...
  if (lobby.match && remaining && lobby.match.phase !== 'match-over') {
    ...
    lobby.match.result = { winnerId: remaining.id, winnerName: remaining.name, reason: 'forfeit' };
    io.to(lobby.code).emit('game:state', publicMatch(lobby.match));
    io.to(lobby.code).emit('game:notice', `${departing?.name || 'Opponent'} disconnected. You win by forfeit.`);
  }
...
setInterval(() => {
  const now = Date.now(); const dt = Math.min((now - lastTick) / 1000, 0.05); lastTick = now;
  lobbies.forEach((lobby) => { if (lobby.match) { tickMatch(lobby.match, dt, now); emitMatch(lobby); } });
}, 1000 / CONFIG.tickRate);

const port = process.env.PORT || 3000;
httpServer.listen(port, () => console.log(`Tag server listening on http://localhost:${port}`));
